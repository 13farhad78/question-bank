...
function checkFinalFields() {
    ...
    const final12thFields = document.getElementById("final-12th-fields");
    const final9thFields = document.getElementById("final-9th-fields");

    // مخفی کردن همه فیلدها در ابتدا
    final12thFields.style.display = "none";
    final9thFields.style.display = "none";

    // نمایش فیلدهای مربوطه در صورت انتخاب نوع نهایی
    if (questionType === "final") {
        if (grade === "12") {
            final12thFields.style.display = "block";
        } else if (grade === "9") {
            final9thFields.style.display = "block";
        }
    }
}
...
function addOption() {
    ...
    optionItem.innerHTML = `
                    <div class="input-with-paste">
                        <input type="text" class="option-input" placeholder="Option text in English" dir="ltr">
